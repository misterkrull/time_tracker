...
class TimeTrackerTimer:
    def __init__(self, id: int, activity_number: int, gui_layer, main_frame: tk.Frame):
        self.id = id
    ...
    def _select_activity(self, _: tk.Event[ttk.Combobox]):
        ...
        self.gui_label.config(
            text=sec_to_time(self.durations_of_activities_in_current_session[self.activity_number])
        )

        for other_timer in self._timer_list:
            ...
            if other_timer.is_running:
                ...
                self.start_button.config(TK_BUTTON_STATES[not is_same_activity])
                return
    ...
    def _start_timer(self) -> None:
        ...
        if all(
            not timer.is_running for timer in self.timer_list
